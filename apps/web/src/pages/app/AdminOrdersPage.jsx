  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          <h1 className="font-heading font-bold text-3xl text-ink tracking-tight">Orders</h1>
        ...
        <div className="flex gap-1.5 flex-wrap">
          <button
            ...
            className={`px-3.5 py-2 rounded-full text-xs font-semibold transition-all ${
              statusFilter === "all" ? "bg-diamond-300 text-ink" : "bg-surface-100 text-ink/60 hover:text-ink"
            }`}
          >
          ...
          {statusChips.map(([s, c]) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 py-2 rounded-full text-xs font-semibold transition-all ${
                  active ? "bg-diamond-300 text-ink" : `${statusColor(s)} hover:bg-diamond-300/10`
                }`}
              >
                {s} ({c})
              </button>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center">
          <Package size={32} className="mx-auto mb-3 text-ink/20" />
                ...
                {filtered.map((o) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <div className="text-sm font-medium text-ink truncate max-w-[240px]">
                        {o.clientName || o.company || `Client #${o.clientId}`}
                      </div>
                    </td>
                    <td className="px-4 py-3 text-sm text-ink/70">{o.patient || "—"}</td>
                    <td className="px-4 py-3 text-xs text-ink/60">{o.department || "—"}</td>
                    <td className="px-4 py-3 text-xs text-ink/60">{o.assignedTo || "—"}</td>
                    <td className="px-4 py-3 text-xs text-ink/60">{formatDate(o.ordered)}</td>
                    <td className="px-4 py-3 text-xs text-ink/60">{formatDate(o.due)}</td>
                    <td className="px-4 py-3">
