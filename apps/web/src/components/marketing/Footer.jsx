...
export function Footer() {
  return (
    <footer className="relative bg-diamond-300 rounded-t-[3rem] md:rounded-t-[4rem] mt-24">
      <div className="section-pad pt-16 pb-8 md:pt-24 md:pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 pb-12 border-b border-white/10">
          ...
          <div className="md:col-span-4">
            <h3 className="font-heading font-bold text-ink text-xl tracking-tight">
            ...
            <div className="mt-6 flex items-center gap-3">
              {SOCIALS.map((s) => (
                <a
                  ...
                  className="w-8 h-8 rounded-lg bg-white/5 border border-ink/20 flex items-center justify-center text-ink/30 hover:text-ink hover:bg-white/10 transition-all duration-300"
                  aria-label={s.label}
                >
                  <s.icon size={14} />
                </a>
            ...
            <div className="mt-4 flex items-center gap-3 font-mono text-xs text-ink/40">
          ...
          <div className="md:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-8">
            {Object.entries(FOOTER_NAV).map(([title, links]) => (
              <div key={title}>
                <h4 className="font-heading font-semibold text-ink/30 text-xs uppercase tracking-widest mb-4">
                  {title}
                </h4>
                <ul className="space-y-2.5">
                  {links.map((link) => (
                    <li key={link.label}>
                      <Link
                        to={link.to}
                        className="group flex items-center gap-1 text-sm text-ink/60 hover:text-ink transition-colors duration-300"
                      >
                        {link.label}
                        <ArrowUpRight
                          size={12}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-8">
          ...
          <div className="flex items-center gap-6 text-ink/30 text-xs">
            <span className="hover:text-ink/60 transition-colors cursor-pointer">
              Privacy Policy
            </span>
            <span className="hover:text-ink/60 transition-colors cursor-pointer">
              Terms of Service
